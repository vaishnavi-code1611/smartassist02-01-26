...
// button submit
// Cross-page scroll using CLASS selectors (no HTML needed)
window.addEventListener("load", () => {
  ...
  const selector = targets[hash] || `#${hash}`;
  const target = document.querySelector(selector);

  if (target) {
    setTimeout(() => {
      const offset = target.offsetTop - 120; // Navbar + padding
      window.scrollTo({ top: offset, behavior: "smooth" });
    }, 800); // Wait for page/animations
  }
});
...
window.addEventListener("load", () => {
  if (window.location.hash) {
    ...
    if (target) {
      ...
      }, 300); // delay allows DOM to fully render
    }
  }
});
